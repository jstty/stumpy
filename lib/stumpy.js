...
Stumpy.prototype.setOptions = function(opts) {
    if(isObject(opts)){
...
Stumpy.prototype._addLogType = function(type) {
    ...
    Stumpy.prototype[type] = function Logger(){
        ...
        if(this._options.display[type]){
            ...
            log.groups = deepCopy(this._data.groups);
        ...
        // fire add handlers
        this._triggerHander('add', 'Log',    log);
        this._triggerHander('add', 'Buffer', log);
        ...
        if(this._options.buffer.size > 0) {
            ...
            if(this._data.buffer.length >= this._options.buffer.size) {
                ...
                this._triggerHander('del', 'Log',    removeLog);
                this._triggerHander('del', 'Buffer', removeLog);
            }
...
// _triggerHander('add', 'Log', log);
Stumpy.prototype._triggerHander = function(trigger, type, log) {
